...
create policy if not exists "Users read own guided study sessions"
  on public.guided_study_sessions for select
  using (auth.uid() = user_id);

create policy if not exists "Users upsert own guided study sessions"
  on public.guided_study_sessions for insert
  with check (auth.uid() = user_id);

create policy if not exists "Users update own guided study sessions"
...
create policy if not exists "Public read pdf cache for public materials"
