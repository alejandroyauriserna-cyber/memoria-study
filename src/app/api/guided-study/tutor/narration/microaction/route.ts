...
const bodySchema = z
  .object({
    action: z
      .enum(["example", "simpler", "casacion", "exam", "repeat_main", "free"])
      .default("free"),
  ...
  .refine(
    (data) =>
      data.action !== "free" || (data.studentMessage && data.studentMessage.trim().length >= 3),
    { message: "La pregunta libre debe tener al menos 3 caracteres." },
  );

function buildAnalysis(body: z.infer<typeof bodySchema>): PageProfessorAnalysis {
  return {
...
export async function POST(request: Request) {
  try {
    ...
    const prompt = `${buildInterruptSystemPrompt()}\n\n${buildInterruptUserPrompt({
      action: body.action,
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Error en interrupción." },
      { status: 500 },
    );
  }
}
