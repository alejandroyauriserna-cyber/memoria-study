...
import { CuadernoAppleNotesLayout } from "@/components/cuaderno/cuaderno-apple-notes-layout";
...
type Tab = "notes" | "dictionary" | "shelf";
type Layout = "apple-notes" | "bookshelf";
...
export function CuadernoWorkspace({
  ...
  const [layout, setLayout] = useState<Layout>("apple-notes"); // Nueva vista por defecto

  return (
    <CuadernoSyncProvider>
      {layout === "apple-notes" ? (
        // Nuevo layout estilo Apple Notes
        <div className="cuaderno-workspace-wrapper">
          <CuadernoAppleNotesLayout
          ...
          <button
            ...
            className="cn-layout-toggle-btn"
            title="Cambiar a vista de estantería"
          >
            📚
          </button>
        </div>
      ) : (
        // Vista clásica de estantería
